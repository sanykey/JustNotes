...
class JustnotesBase(sublime_plugin.TextCommand):
...
class JustnotesNewTaskCommand(JustnotesBase):
    def runCommand(self, edit):
        ...
        for i, line in enumerate(regions):
            ...
            if 'item' in current_scope:
                ...
                print('oops, need to improve JustNotesNewCommand')
            if eol:
...
class JustnotesCompleteTaskCommand(JustnotesBase):
    def runCommand(self, edit):
...
class JustnotesCancelTaskCommand(JustnotesBase):
    def runCommand(self, edit):
        original = [r for r in self.view.sel()]
        try:
        ...
        for line in regions:
            ...
            if 'pending' in current_scope:
                ...
                if started_matches:
                    ...
                    JustnotesCancelTaskCommand.calc_end_start_time(self, edit, line, started_matches.group(1), toggle_matches, canc_line_end, eol, tag='wasted')
            elif 'header' in current_scope:
                eol = self.view.insert(edit, line.end(), canc_line_end)
                if started_matches:
                    JustnotesCancelTaskCommand.calc_end_start_time(self, edit, line, started_matches.group(1), toggle_matches, canc_line_end, eol, tag='wasted')
                ...
                # grps = done_matches.groups()
                # parentheses = JustnotesCancelTaskCommand.check_parentheses(self.date_format, grps[4] or '')
...
class PlainTasksArchiveCommand(JustnotesBase):
    ...
    def get_task_note(self, task, tasks):
        ...
        while self.view.scope_name(note_line) == 'text.note':
...
class PlainTasksSortByDate(JustnotesBase):
    def runCommand(self, edit):
        archive_pos = self.view.find(self.archive_name, 0, sublime.LITERAL)
        if archive_pos:
...
class PlainTasksStatsStatus(sublime_plugin.EventListener):
    def on_activated(self, view):
        if not view.score_selector(0, "text.note") > 0:
            return
        self.set_stats(view)

    @staticmethod
    def set_stats(view):
        view.set_status('Notes', PlainTasksStatsStatus.get_stats(view))
    ...
    def get_stats(view):
        ...
        tasks_dates = [JustnotesCancelTaskCommand.check_parentheses(date_format, t, is_date=True) for t in tasks_dates]
...
class PlainTasksCopyStats(sublime_plugin.TextCommand):
    def is_enabled(self):
        return self.view.score_selector(0, "text.note") > 0

    def run(self, edit):
        msg = self.view.get_status('Notes')
...
class PlainTasksArchiveOrgCommand(JustnotesBase):
    def runCommand(self, edit):
