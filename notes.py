...
def get_root():
    project_settings = sublime.active_window().active_view().settings().get('JustNotes')
    if project_settings:
...
class NoteChangeColorCommand(sublime_plugin.WindowCommand):
    ...
    def run(self):
        ...
        current_color = os.path.basename(self.original_cs).replace("JustNotes-","").replace(".hidden-tmTheme", "")
        if ST3:
    ...
    def on_select(self, index):
        ...
        if index == -1:
            ...
            try:
                path = sublime.find_resources("JustNotes-" + self.colors[index] + ".hidden-tmTheme")
                path = path[0]
            except:
                path = os.path.join("Packages" , "JustNotes", "Color Schemes", "JustNotes-" + self.colors[index] + ".hidden-tmTheme")
